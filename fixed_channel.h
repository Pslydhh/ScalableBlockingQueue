    ...
    bool put(T v) {
        for (;;) {
            ...
            if constexpr (blocking) {
                ...
                while ((put_version_field_local =
                            LOADcs(&c->put_version_field)) < version) {
                    ob_futex_wait(&c->put_version_field,
                                  put_version_field_local);
                }
            } else {
                if ((LOADcs(&c->put_version_field)) < version) {
                    return false;
                } else if (!CAScs(&put_index, &put_index_local,
                                  put_index_local + 1)) {
                    continue;
                }
            }

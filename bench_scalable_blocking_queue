...
void test_use_mutex(int count, int num, int num2, int scores) {
...

    {
        ...
        std::cout << "mutex elapsed time is " << elapsedTime.count() << " milliseconds"
                  << std::endl;
    }
}
...
void test_new_blocking_queue(int count, int num, int num2, int scores) {
    ...
    {
        auto beginTime = std::chrono::high_resolution_clock::now();
        ScalableBlockingQueue<int>* q = &qq;
        ...
        auto elapsedTime =
                std::chrono::duration_cast<std::chrono::milliseconds>(endTime - beginTime);
        std::cout << "new blocking queue elapsed time is " << elapsedTime.count() << " milliseconds"
                  << std::endl;
    }
}
